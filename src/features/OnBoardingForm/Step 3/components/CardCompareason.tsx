...
const CardCompareason = () => {
  return (
    <Card className="border-border/70 w-full bg-white dark:bg-black shadow-sm py-3">
        ...
        <Tabs defaultValue={tabs[0].value} className="w-full max-w-sm">
          ...
          {tabs.map((tab) => (
            <TabsContent key={tab.value} value={tab.value}>
              <p className="text-muted-foreground p-2 text-sm leading-6">{tab.content}</p>
            </TabsContent>
          ))}
        </Tabs>
      </CardContent>
...
export default CardCompareason;
